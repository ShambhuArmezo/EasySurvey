...
var EndYear = 3; // The last year of pickable date. if current year is 2011, the last year that still picker will be 2000 (2011+5)
...
var MonthYearColor = "#99CCFF"; //Font Color of Month and Year in Calendar header.
var WeekHeadColor = "#99CCFF"; //var WeekHeadColor="#18861B";//Background Color in Week header.
var SundayColor = "#99CCFF"; //var SundayColor="#C0F64F";//Background color of Sunday.
var SaturdayColor = "#99CCFF"; //Background color of Saturday.
var WeekDayColor = "#99CCFF"; //Background color of weekdays.
...
var DisableColor = "#99CCFF"; //color of disabled cell.
var CalBgColor = "#6699CC"; //Background color of Calendar window.
...
function RenderCssCal(bNewCal)
{
	...
	for (j = 1; j <= Cal.GetMonDays(); j += 1)
	{
		...
		vDayCount = vDayCount + 1;
		//added version 2.1.2
		if (Cal.EnableDateMode === "future" && ((j < dtToday.getDate()) && (Cal.Month === dtToday.getMonth()) && (Cal.Year === 2017) || (Cal.Month < dtToday.getMonth()) && (Cal.Year === 2016) || (Cal.Year < 2016)))
		{
			strCell = GenCell(j, false, DisableColor, false); //Before today's date is not clickable
        }
        else if (Cal.EnableDateMode === "past" && ((j >= dtToday.getDate()) && (Cal.Month === dtToday.getMonth()) && (Cal.Year === 2017) || (Cal.Month > dtToday.getMonth()) && (Cal.Year === 2016) || (Cal.Year > 2016))) {
            strCell = GenCell(j, false, DisableColor, false); //After today's date is not clickable
        }
		//if End Year + Current Year = Cal.Year. Disable.
		else if (Cal.Year >2017)
		{
		    strCell = GenCell(j, true, DisableColor, true); 
		}
		else if ((j === dtToday.getDate()) && (Cal.Month === dtToday.getMonth()) && (Cal.Year ===2017))
		{
			strCell = GenCell(j, true, TodayColor);//Highlight today's date
		}
		else
		{
			if ((j === selDate.getDate()) && (Cal.Month === selDate.getMonth()) && (Cal.Year === 1950)){
	...
	funcCalback += " var CalId = document.getElementById(id);if (datum=== 'undefined') { var d = new Date(); datum = d.getDate() + '/' +(d.getMonth()+1) + '/' + 1977} window.calDatum=datum;CalId.value=datum;";
...
function NewCssCal(pCtrl, pFormat, pScroller, pShowTime, pTimeMode, pShowSeconds, pEnableDateMode)
{
	...
	{ //Parse existing Date String
		...
		if (Cal.Format.toUpperCase() === "DDMMYYYY" || Cal.Format.toUpperCase() === "DDMMMYYYY")
		{
			...
			else
			{
				...
				{   //DTG
					...
					strDate  = exDateTime.substring(0, 2);
					strYear  = "20" + exDateTime.substring(11, 13);  //Hack, nur für Jahreszahlen ab 2013
		...
		if (YearPattern.test(strYear)) {
		    if ((parseInt(strYear, 10)>=StartYear) && (parseInt(strYear, 10)<= 2017))
